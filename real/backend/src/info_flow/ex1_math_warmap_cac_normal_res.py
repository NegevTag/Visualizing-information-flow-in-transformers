...
def calc_contribution_per_layer_per_residual(model: nnsight.LanguageModel, prompt: str, remote: bool | str = True):
    ...
    with model.trace(prompt, remote=remote):
        ...
        for l in range(LAYERS_NUM):
            ...
            per_layer_contribution_per_residual.append(contriubution_per_residual.save()) #(query,key,d_model)
        print(per_layer_contribution_per_residual)
        per_layer_contribution_per_residual.save()

    # Dump layer-0 attention-per-key contribution for offline inspection.
    _dump_dir = Path(__file__).resolve().parent / ".cache"
    _dump_dir.mkdir(exist_ok=True)
    _dump_path = _dump_dir / "attention_per_key_L0.pt"
    _l0 = per_layer_contribution_per_residual[0].detach().cpu()
    torch.save(_l0, _dump_path)
    print(f"[dump] attention_per_key (L0) -> {_dump_path}  shape={tuple(_l0.shape)}")
